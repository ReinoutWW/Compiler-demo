...
public:
    ...
    void gen_term(const NodeTerm* term) {
        struct TermVisitor {
            Generator& gen;
            void operator()(NodeTermIntLit* term_int_lit) const {
            ...
            void operator()(NodeTermIdent* term_ident) const {
                ...
                if (it == gen.m_vars.cend()) {
                    std::cerr << "Identifier already used: " << term_ident->ident.value.value() << std::endl;
                    exit(EXIT_FAILURE);
                }
            ...
            void operator()(NodeTermParen* node_term_paren) const {
                gen.gen_expr(node_term_paren->expr);
            }
        };
